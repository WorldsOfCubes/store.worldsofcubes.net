...
class Profile extends View {
	...
	public function Show($modal_mode = true) {
		...
		$user_info['money'] = array($this->user->getMoney().$donate['currency_donate'], 'Внесенные деньги');

		if ($this->admin_mode && !$this->self_ignore) {
			if($this->user->getEcon()) $user_info['econ'] = array($this->user->getEcon().$donate['currency_ingame'], 'Заработанные, но не выведенные деньги');
