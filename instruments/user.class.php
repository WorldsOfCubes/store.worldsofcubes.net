...
Class User {
	...
	public function login($tmp, $ip, $save = false) {
		...
		if ($save)
			setcookie("WoCAccountCookie", $tmp, time() + 60 * 60 * 24 * 30 * 12, '/', '.' . str_replace('store.', '', $_SERVER['SERVER_NAME']));
	...
	public function logout() {
		...
		if (isset($_COOKIE['WoCAccountCookie']))
			setcookie("WoCAccountCookie", "", time() - 3600, '/', '.' . str_replace('store.', '', $_SERVER['SERVER_NAME']));
	}
